...
|                                       |                                                                                                                                                                                                                                                                                                          |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Extension** (`src/extension`)       | MV3 service worker: capture, coalescing, id mapping, an outbound queue, one WebSocket, an auth watcher, a command router and the window restorer. `dashboard.html` is a full page; it talks to the worker over one `runtime.connect` port. `lazy.html` stands in for a tab that has not been loaded yet. |
| **Worker** (`src/worker`)             | On the user's own custom domain. Verifies the Cloudflare Access JWT on every route - the WebSocket upgrade included - then hands the request to `env.USER_HUB.getByName(sub)`.                                                                                                                           |
| **`UserHub`** (`src/worker/user-hub`) | One SQLite Durable Object per user. The authoritative op log, the sockets (tagged by device id), and a daily prune alarm.                                                                                                                                                                                |
